...
const FAQSection = () => {
    ...
    <section className="py-24 px-6 lg:px-8 bg-background relative overflow-hidden" id="faq">
      ...
      <div className="max-w-4xl mx-auto relative z-10">
        ...
        >
          ...
          >
            <span className="text-primary text-white text-base font-bold tracking-wide">FAQ</span>
          </motion.div>
          ...
          <motion.p 
            className="text-xl text-white opacity-70  mx-auto leading-relaxed font-medium"
  ...
  {faqs.map((faq, index) => (
    ...
    >
      <AccordionItem 
        value={`item-${index}`}
        className="bg-card/60 backdrop-blur-sm border border-white/30 rounded-2xl px-8 
                   transition-all duration-500 group
                   hover:border-white/70 hover:shadow-[0_0_20px_rgba(255,255,255,0.3)]
                   data-[state=open]:border-purple-500 
                   data-[state=open]:shadow-[0_0_25px_rgba(168,85,247,0.6)]"
      >
        <AccordionTrigger className="text-left text-white transition-colors duration-300 text-lg md:text-xl font-semibold py-8 group-hover:scale-[1.02]">
          <motion.span
            whileHover={{ x: 10 }}
        ...
        <AccordionContent className="pb-8 text-base md:text-lg leading-relaxed font-medium text-white/50">
          <motion.div
